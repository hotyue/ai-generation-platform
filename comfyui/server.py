...
class PromptServer():
    def __init__(self, loop):
        ...
        # ===== EVENT EMIT PATCH BEGIN =====
        platform_event_endpoint = os.getenv("PLATFORM_EVENT_ENDPOINT")

        if platform_event_endpoint:
            def emit_prompt_event(prompt_id: str, phase: str, payload: dict):
                data = {
                    "source": "comfyui",
                    "compute_node": os.getenv("COMPUTE_NODE", "unknown"),
                    "prompt_id": prompt_id,
                    "phase": phase,
            ...
            # 绑定到 server 实例（不是类）
            self.emit_prompt_event = emit_prompt_event
        # ===== EVENT EMIT PATCH END =====
