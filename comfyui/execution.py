...
class PromptExecutor:
    ...
    async def execute_async(self, prompt, prompt_id, extra_data={}, execute_outputs=[]):
        ...
        # ===== STEP 3-C' · running（事实：真正开始执行）=====
        if hasattr(self.server, "emit_prompt_event"):
            try:
                self.server.emit_prompt_event(
                    prompt_id=prompt_id,
                    phase="running",
                    payload={
...
class PromptQueue:
    def __init__(self, server):
        ...
        self.flags = {}
        # ===== 事实事件钩子（可选、旁路）=====
        self._emit_prompt_event = getattr(server, "emit_prompt_event", None)

    def put(self, item):
        with self.mutex:
            heapq.heappush(self.queue, item)

            # ===== STEP 3-B · queued（事实：Prompt 已入队）=====
            if self._emit_prompt_event is not None:
                try:
                    # item 结构：(priority, prompt_id, prompt, extra_data, outputs)
    ...
    def task_done(self, item_id, history_result,
        ...
        with self.mutex:
            ...
            # ---- STEP 3-D · finished 事实事件（只读、不可反向影响） ----
            try:
                history_payload = (
                    history_result if isinstance(history_result, dict) else {}
                )
                ...
                self.server.emit_prompt_event(
                    ...
                    payload={
                        "item_id": item_id,
                        "status": status_dict,  # success / error
                        "outputs": list(history_payload.get("outputs", {}).keys()),
                    },
                )
            except Exception:
                # ⚠️ 插桩不得影响任何官方执行路径
